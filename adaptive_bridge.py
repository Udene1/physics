...
def _request(path, student, method="GET", payload=None):
    ...
    if method == "GET":
        ...
        url = f"{url}{separator}student={__import__('urllib.parse').parse.quote(student)}"
...
def snapshot(student):
    return _request("/v1/snapshot", student)


def intervention(student):
    return _request("/v1/intervention", student)


def timeline(student):
    return _request("/v1/timeline", student)


def submit_remediation(student, intervention_id, reasoning, answer, confidence=None, hint_used=False, duration_seconds=None):
    return _request("/v1/remediation", student, "POST", {
