...
BEGIN
  IF NEW.school_id = OLD.school_id THEN
    RETURN NEW;
  END IF;
  IF EXISTS (SELECT 1 FROM classroom_staff WHERE staff_id = OLD.id) THEN
...
INSERT INTO schema_migrations(version) VALUES ('017_school_staff_integrity') ON CONFLICT (version) DO NOTHING;
