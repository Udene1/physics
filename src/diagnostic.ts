...
export class DiagnosticEngine{
 ...
 evaluate(studentId:number,observations:DiagnosticObservation[]):DiagnosticResult{return this.store.transaction(()=>{
  ...
  const scores:Record<string,number>={};const confidence:Record<string,number>={};const grouped=new Map<string,Array<{score:number;kind:string;difficulty:number}>>();
  ...
  for(const[conceptId,list]of grouped){const weighted=list.reduce((sum,o)=>sum+o.score*weightFor(o.kind as EvidenceKind,o.difficulty),0);const totalWeight=list.reduce((sum,o)=>sum+weightFor(o.kind as EvidenceKind,o.difficulty),0);scores[conceptId]=Math.round((weighted/totalWeight)*10)/10;confidence[conceptId]=Math.round(Math.min(1,totalWeight/3)*100)/100;this.store.setMastery(studentId,conceptId,scores[conceptId]);}
