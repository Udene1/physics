...
export async function getPostgresLearnerTimeline(store:LearningStoreContract&{query(sql:string,params?:unknown[]):Promise<{rows:Row[]}>},studentId:number):Promise<LearnerTimelineEvent[]>{
 ...
 return result.rows.map(x=>{
  ...
  const eventType=String(x.event_type);const aggregate=String(x.aggregate_type);const conceptId=x.concept_id==null?null:String(x.concept_id);const problemId=payload.problemId==null?null:String(payload.problemId);const score=payload.checkpointScore==null?(payload.value==null?null:Number(payload.value)):Number(payload.checkpointScore);
  ...
  if(aggregate==='intervention')return{id:`event:${x.id}`,type:'intervention' as const,at:String(x.created_at),conceptId,problemId,summary:`Intervention ${payload.status??payload.stage??'state changed'}`,verdict:payload.status==null?null:String(payload.status),score:null};
