...
export async function getClassroomIntelligence(pool: Pool, classroomId: number): Promise<ClassroomIntelligence> {
  ...
  const bottlenecks = mastery.rows.map((r) => ({ conceptId: String(r.concept_id), averageMastery: number(r.average_score), learnersAttempted: number(r.learners_attempted) })).map((r) => ({
    conceptId: r.conceptId,
    affectedLearners: r.learnersAttempted,
    averageMastery: r.averageMastery,
    reason: r.averageMastery < 60 ? 'class mastery is below 60%' : r.learnersAttempted < totalStudents * 0.5 ? 'fewer than half the class has demonstrated evidence' : '',
  })).filter((r) => r.reason);
  return {
...
export async function createClassroom(pool: Pool, name: string, code: string): Promise<number> {
  const result = await pool.query('INSERT INTO classrooms(name,code) VALUES($1,$2) RETURNING id', [name.trim(), code.trim().toUpperCase()]);
