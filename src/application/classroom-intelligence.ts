...
export async function getClassroomIntelligence(pool: Pool, classroomId: number): Promise<ClassroomIntelligence> {
  const classroom = await pool.query(
    'SELECT id,name,code FROM classrooms WHERE id=$1',
    [classroomId],
  );
  ...
  const [roster, mastery, misconceptions, remediation, attention] = await Promise.all([
    ...
    pool.query(`
      ...
      ORDER BY m.concept_id`, [classroomId]),
    pool.query(`
      ...
      FROM classroom_students cs
      JOIN misconceptions m ON m.student_id=cs.student_id
      LEFT JOIN misconception_state ms ON ms.misconception_id=m.id
      WHERE cs.classroom_id=$1 AND m.status='active'
      GROUP BY m.code,m.concept_id
      ORDER BY affected_learners DESC,total_occurrences DESC`, [classroomId]),
    pool.query(`
      SELECT ra.outcome, COUNT(*)::int AS count
      FROM classroom_students cs
      JOIN review_attempts ra ON ra.student_id=cs.student_id
      WHERE cs.classroom_id=$1
      GROUP BY ra.outcome`, [classroomId]),
    pool.query(`
      ...
      LEFT JOIN misconception_state ms ON ms.misconception_id=m.id
      LEFT JOIN concept_reviews cr ON cr.student_id=cs.student_id
      WHERE cs.classroom_id=$1
      GROUP BY cs.student_id
      ORDER BY max_confidence DESC NULLS LAST,max_severity DESC NULLS LAST,cs.student_id`, [classroomId]),
  ]);

  const outcomeCounts = new Map<string, number>(remediation.rows.map((r) => [String(r.outcome), number(r.count)]));
  const repaired = number(outcomeCounts.get('retained'));
  const stillPresent = number(outcomeCounts.get('misconception_reopened'));
  ...
  return {
    ...
    mastery: mastery.rows.map((r) => ({
      conceptId: String(r.concept_id),
      averageScore: number(r.average_score),
      learnersAttempted: number(r.learners_attempted),
    })),
    misconceptions: misconceptions.rows.map((r) => ({
      code: String(r.code),
      conceptId: String(r.concept_id),
      affectedLearners: number(r.affected_learners),
      totalOccurrences: number(r.total_occurrences),
      averageConfidence: number(r.average_confidence),
    })),
    interventionEffectiveness: {
      attempts,
      repaired,
    ...
    attention: attention.rows
      .map((r) => {
        ...
        const level: 'high' | 'medium' = reasons.length >= 2 ? 'high' : 'medium';
        return {
...
export async function createClassroom(pool: Pool, name: string, code: string): Promise<number> {
  const result = await pool.query(
    'INSERT INTO classrooms(name,code) VALUES($1,$2) RETURNING id',
