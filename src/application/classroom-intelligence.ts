...
export async function getClassroomIntelligence(pool: Pool, classroomId: number): Promise<ClassroomIntelligence> {
  ...
  const [roster, mastery, misconceptions, remediation, attention, strategies, readiness] = await Promise.all([
    ...
    pool.query(`SELECT cs.student_id FROM classroom_students cs JOIN mastery m ON m.student_id=cs.student_id
      WHERE cs.classroom_id=$1 AND m.score>=80 AND m.attempts>0 GROUP BY cs.student_id
      HAVING COUNT(*) FILTER (WHERE m.score>=80) = COUNT(*) AND NOT EXISTS
        (SELECT 1 FROM misconceptions mx WHERE mx.student_id=cs.student_id AND mx.status='active') ORDER BY cs.student_id`, [classroomId]),
  ]);
  ...
  const bottlenecks = mastery.rows.map((r) => ({ conceptId: String(r.concept_id), affectedLearners: number(r.learners_attempted), averageMastery: number(r.average_score) })).map((r) => ({ ...r, reason: r.averageMastery < 60 ? 'class mastery is below 60%' : r.affectedLearners < totalStudents * 0.5 ? 'fewer than half the class has demonstrated evidence' : '' })).filter((r) => r.reason);
  return {
    ...
    readyToAdvance: readiness.rows.map((r) => number(r.student_id)), bottlenecks,
  };
}
