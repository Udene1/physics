...
export const ANSWER_SPECS: Readonly<Record<string, ProblemAnswerSpec>> = {
  'force-motion-discrimination-1': {criteria:[numeric('net-force',0,'N'),concept('causal-model',/motion.*not.*require.*force/i,/does not.*need.*force/i,/not.*need.*force.*(?:sustain|maintain)/i,/not.*(?:needed|required).*to.*(?:sustain|maintain).*motion/i)]},
  ...
  'energy-review-1': {criteria:[numeric('work',50,'J'),concept('quantity-distinction',/N.*(?:force|work).*J/i,/force.*(?:not|while).*work/i)]},
