...
export class LearningEngine {
  ...
  recordStructuredAttempt(studentId:number, conceptId:string, input:AttemptInput): LearnerSnapshot {
    ...
    for (const finding of findings) this.store.upsertMisconception(studentId, conceptId, finding.code, finding.severity, evidenceId);
    this.store.saveSession(studentId, 'learning', conceptId, 0);
    return this.snapshot(studentId);
  }
  diagnoseAttempt(conceptId:string, reasoning:string): ReturnType<typeof diagnoseReasoning> {
