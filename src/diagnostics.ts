...
const RULES:readonly Rule[]=[
  ...
  {code:'force_causes_motion',severity:4,patterns:[/force.{0,30}(causes|creates|produces|makes|make).{0,30}(motion|velocity|move|moving)/i,/no\s+force.{0,50}(means|mean|so).{0,50}(no\s+motion|stopped|cannot\s+keep\s+moving|cannot\s+move)/i,/moving.{0,30}(means|must|needs|requires|need).{0,30}force/i,/moving.{0,30}(need|requires).{0,30}force/i],remediation:'Repair Newtonian reasoning: distinguish velocity from acceleration and ask whether net force is zero or nonzero.'},
