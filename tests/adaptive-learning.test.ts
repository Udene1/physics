...
test('remediation requires reasoning checkpoints, not only a correct answer', () => {
  ...
  const weak = engine.submitRemediationAttempt(student, intervention.id, {
    correct:true,
    reasoning:'0 N.',
    answer:'0 N',
  });
  ...
  const strong = engine.submitRemediationAttempt(student, intervention.id, {
    correct:true,
    reasoning:'The velocity is constant, so acceleration is zero. Using F_net = ma gives net force zero. A net force changes velocity by producing acceleration; it is not needed to sustain constant velocity.',
    answer:'0 N',
  });
...
test('persistent failure raises misconception confidence and successful transfer reduces it', () => {
  ...
  const repaired = engine.submitRemediationAttempt(student, first.id, {
    correct:true,
    reasoning:'Constant velocity means acceleration is zero. F_net = ma, so the net force is zero. Force changes velocity through acceleration, not by sustaining motion.',
    answer:'0 N',
  });
...
test('strong demonstrated repair creates performance-based spaced review', () => {
  ...
  engine.submitRemediationAttempt(student, discrimination.id, {
    correct:true,
    reasoning:'Constant velocity means acceleration is zero. F_net = ma, so net force is zero. A net force changes velocity by causing acceleration.',
    answer:'0 N',
  });
  const transfer = engine.nextIntervention(student)!;
  const result = engine.submitRemediationAttempt(student, transfer.id, {
    correct:true,
    reasoning:'F_net = ma gives a = 4/2 = 2 m/s² east. Then v = u + at = 3 + 2(2) = 7 m/s east. The force changes velocity through acceleration.',
