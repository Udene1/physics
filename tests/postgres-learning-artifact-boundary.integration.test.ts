...
import { recordArtifactAndUsage, recordGeneratedArtifact } from '../src/infrastructure/learning-artifacts.js';
...
test('generated artifact boundary persists provenance and usage atomically', async (t) => {
  ...
  try {
    ...
    const artifact = await recordGeneratedArtifact(pool, {
      studentId,
      artifactKind: 'remediation_problem',
      content: { problemId: 'force-motion-transfer-1' },
      generationContext: { misconception: 'force-causes-motion' },
      modelProvider: 'provider', modelName: 'model', modelVersion: 'v1',
      promptTemplateVersion: 'remediation-v1', curriculumVersion: 'physics-v1',
    });
    ...
    const evidence = await pool.query(
      `INSERT INTO evidence(student_id,concept_id,kind,problem_id,reasoning) VALUES($1,'forces','remediation_attempt','force-motion-transfer-1','F_net = ma') RETURNING id`,
      [studentId],
    );
    await recordArtifactAndUsage(pool, {
      studentId,
      artifactKind: 'review_problem',
      content: { problemId: 'force-motion-transfer-1' },
    }, 'presented', Number(evidence.rows[0].id));

    const usage = await pool.query(
      `SELECT u.usage_kind,u.student_id,u.evidence_id,a.artifact_kind FROM learning_artifact_usage u JOIN learning_artifacts a ON a.id=u.artifact_id WHERE u.student_id=$1 ORDER BY u.id`,
      [studentId],
    );
