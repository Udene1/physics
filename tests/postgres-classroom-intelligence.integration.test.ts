...
test('classroom intelligence identifies cohort patterns and learner attention', async (t) => {
  ...
  try {
    await enrollStudent(pool, classroomId, studentOne);
    await enrollStudent(pool, classroomId, studentTwo);
    ...
    assert.deepEqual(summary.mastery[0], { conceptId: 'forces', averageScore: 60, learnersAttempted: 2 });
    assert.equal(summary.misconceptions[0].code, 'force-causes-motion');
    assert.equal(summary.misconceptions[0].affectedLearners, 2);
    assert.equal(summary.interventionEffectiveness.attempts, 1);
    assert.equal(summary.interventionEffectiveness.repaired, 1);
    assert.equal(summary.interventionEffectiveness.repairRate, 1);
    assert.ok(summary.attention.some((x) => x.studentId === studentOne && x.attention === 'high'));
  } finally {
...
test('classroom enrollment is idempotent', async (t) => {
  ...
  try {
    ...
    await enrollStudent(pool, classroomId, studentId);
    const result = await pool.query('SELECT count(*)::int AS count FROM classroom_students WHERE classroom_id=$1 AND student_id=$2', [classroomId, studentId]);
