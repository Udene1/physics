...
test('math is a dependency layer, not the learning spine', () => { const mastery={arithmetic:100,algebra:100,geometry:100,graphs:100,vectors:100,measurement:100}; assert.ok(missingRequirements(mastery,'motion').length === 0); assert.ok(learningFrontier(mastery).some(c => c.id === 'motion')); });
